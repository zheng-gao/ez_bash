...
function ez.json.flatten {
    ...
    local ignore_fields && ez.function.arguments.get_list "ignore_fields" "$(ez.argument.get --short "-i" --long "--ignore-fields" --arguments "${@}")" &&
    ...
    if [[ -n "${ignore_fields[*]}" ]]; then for f in "${ignore_fields[@]}"; do ez.array.delete.item "fields" "${f}"; done; fi
    ...
    echo -e "$(ez.text.format -f "Yellow")$({
        ...
        jq -r "${list_filter}[$(ez.join ", " "${fields[@]}")] | @tsv | tostring" <<< "${data}" | sed "s/\t/|/g" | sed "s/||/| |/g" | sed "s/||/| |/g" \
