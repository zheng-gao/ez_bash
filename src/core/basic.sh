...
function ez.string.format { # ${1} = format, ${2} ~ ${n} = ${input_string[@]}
    if [[ -z "${1}" || "${1}" = "-h" || "${1}" = "--help" ]]; then
        local s="    " f; echo; echo "${s}[Usage]"; echo "${s}${FUNCNAME[0]} [Format] [String]"; echo; echo "${s}[Demo]${s}[Format]"; 
        for f in "${!EZ_FORMAT_SET[@]}"; do echo -e "${s}${EZ_FORMAT_SET[${f}]}demo${EZ_FORMAT_SET[ResetAll]}${s}${f}"; done; echo; return 0
    fi
...
function ez.join { local d="${1}" o i; for i in "${@:2}"; do [[ -z "${o}" ]] && o="${i}" || o+="${d}${i}"; done; echo "${o}"; }
