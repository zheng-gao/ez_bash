function ezb_random_int() {
    ...
    local lower_bound; lower_bound="$(ezb_arg_get --short "-l" --long "--lower-bound" --arguments "${@}")"; [ "${?}" -ne 0 ] && return 1
    local upper_bound; upper_bound="$(ezb_arg_get --short "-u" --long "--upper-bound" --arguments "${@}")"; [ "${?}" -ne 0 ] && return 1
