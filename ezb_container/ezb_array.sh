function ez_split_string_into_array() {
    local usage_string=$(ezb_build_usage -o "init" -a "ez_split_string_into_array" -d "Split string by delimiter")
    usage_string+=$(ezb_build_usage -o "add" -a "-d|--delimiter" -d "Given delimiter, default = \",\"")
    usage_string+=$(ezb_build_usage -o "add" -a "-s|--string" -d "Given string")
...
function ez_print_array_with_delimiter() {
    local usage_string=$(ezb_build_usage -o "init" -a "ez_print_array_with_delimiter" -d "Print array with delimiter")
    usage_string+=$(ezb_build_usage -o "add" -a "-d|--delimiter" -d "Given Delimiter")
    usage_string+=$(ezb_build_usage -o "add" -a "-a|--array" -d "Given Array Item_1 Item_2 ...")
    if [[ "${1}" == "" ]] || [[ "${1}" == "-h" ]] || [[ "${1}" == "--help" ]]; then ezb_print_usage "${usage_string}"; return 1; fi
    local delimiter=""
    local array=()
    while [[ ! -z "${1-}" ]]; do
...
function ez_check_item_in_array() {
    local usage_string=$(ezb_build_usage -o "init" -a "ez_check_item_in_array" -d "Check if an item is in an array")
    usage_string+=$(ezb_build_usage -o "add" -a "-i|--item" -d "Given Item")
    usage_string+=$(ezb_build_usage -o "add" -a "-a|--array" -d "Given Array Item_1 Item_2 ...")
...
function ez_get_diff_between_two_sets() {
    ...
    local usage_string=$(ezb_build_usage -o "init" -a "ez_get_diff_between_two_arrays" -d "Get the differences between two sets")
...
function ez_is_subset() {
    ...
    local valid_operation_string=$(ez_print_array_with_delimiter -d ", " -a "${valid_operation[@]}")
    local usage_string=$(ezb_build_usage -o "init" -a "ez_is_subset" -d "Check if a set is the subset of another set, Null is a subset of All sets")
    usage_string+=$(ezb_build_usage -o "add" -a "-o|--operation" -d "[${valid_operation_string}], default = ${valid_operation[0]}")
